...
def process_batch_for_attr_patching_resid_pre(
    ...
    for i, (single_clean_tokens, single_corrupted_tokens, single_answer_token_indices) in tqdm(enumerate(zip(clean_tokens, corrupted_tokens, answer_token_indices)), total=num_examples):
        ...
        _, clean_cache, _ = get_cache_fwd_and_bwd(
...
for PROMPTS_SET in range(0, 30):
    prompts_and_answers = pickleload(f"/workspace/removing-layer-norm/mech_interp/experiments/attribution_patching/data/prompts_{PROMPTS_SET}/prompts.pkl")
    ...
    for MODEL_NAME in ['baseline', 'vanilla', 'noLN']: 
        ...
        store_location = f"/workspace/removing-layer-norm/mech_interp/experiments/attribution_patching/data/prompts_{PROMPTS_SET}/new_resid_pre_act_patch_result_{MODEL_NAME}.pkl"
        ...
        store_location = f"/workspace/removing-layer-norm/mech_interp/experiments/attribution_patching/data/prompts_{PROMPTS_SET}/new_resid_pre_attr_patch_result_{MODEL_NAME}.pkl"
