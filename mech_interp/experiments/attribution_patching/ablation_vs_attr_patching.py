...
# No need to modify the corrupted_cache since we're using it directly
# ActivationCache objects don't support item assignment
# We'll just use the corrupted_cache as is
from copy import deepcopy
ablated_cache = deepcopy(corrupted_cache)
ablated_cache.cache_dict['blocks.11.hook_mlp_out'] = ablated_cache.cache_dict['blocks.11.hook_mlp_out'] * 0
# Or even more concisely using the tuple notation:
from transformer_lens import patching
every_block_act_patch_result = patching.get_act_patch_block_every(
    model, corrupted_tokens, ablated_cache, metric=ioi_metric
)
imshow(
...
plt.title("FINETUNED MODEL: Patching comparison in each block residual stream")
plt.show()
#%%
