...
for set_prompts in [1]:
    for model, model_name in zip([load_nln_model(), load_finetuned_model()], ["nln", "finetuned"]):
        ...
        if set_prompts == 1:
            ...
            prompts = [
                "When Alex and Sarah went to the library, Alex gave the book to",
                "When Alex and Sarah went to the library, Sarah gave the book to",
                "When Mike and Lisa went to the restaurant, Lisa gave the menu to",
                "When Mike and Lisa went to the restaurant, Mike gave the menu to",
                "When David and Emma went to the beach, Emma gave the towel to",
                "When David and Emma went to the beach, David gave the towel to",
                "After Kevin and Rachel went to the concert, Rachel gave a ticket to",
                "After Kevin and Rachel went to the concert, Kevin gave a ticket to",
            ]
            answers = [
                (" Sarah", " Alex"),
                (" Alex", " Sarah"),
        ...
        ### OK NOW WE CAN DO THE ACTIVATION PATCHING
...
#%%
for set_prompts in [1, 2]:
    every_block_act_patch_result_finetuned = torch.load(f"every_block_act_patch_result_finetuned_set_{set_prompts}.pt")
    every_block_attr_patch_result_finetuned = torch.load(f"every_block_attr_patch_result_finetuned_set_{set_prompts}.pt")
    every_block_act_patch_result_nln = torch.load(f"every_block_act_patch_result_nln_set_{set_prompts}.pt")
    every_block_attr_patch_result_nln = torch.load(f"every_block_attr_patch_result_nln_set_{set_prompts}.pt")
    finetuned_l1 = torch.abs(every_block_act_patch_result_finetuned - every_block_attr_patch_result_finetuned)
    nln_l1 = torch.abs(every_block_act_patch_result_nln - every_block_attr_patch_result_nln)

    finetuned_l1.shape
    nln_l1.shape
    fig, axs = plt.subplots(3, 1, figsize=(10, 15))
    for i, block in enumerate(('resid_pre', 'attn_out', 'mlp_out')):
        axs[i].plot(finetuned_l1[i].cpu().numpy().sum(axis=-1))
        axs[i].plot(nln_l1[i].cpu().numpy().sum(axis=-1))
        axs[i].set_title(f"L1 Loss of each block {block}")
        axs[i].set_xlabel("Layer")
        axs[i].set_ylabel("L1 Loss")
        axs[i].legend(["Finetuned", "NLN"])
    ...
    for i, block in enumerate(('resid_pre', 'attn_out', 'mlp_out')):
        ...
        plt.scatter(finetuned_l1[i].cpu().numpy().flatten(), nln_l1[i].cpu().numpy().flatten(), label=block)
        plt.ylabel("L1 Loss of NLN")
        plt.xlabel("L1 Loss of Finetuned")
        plt.axvline(0, color='black', linestyle='--')
        plt.axhline(0, color='black', linestyle='--')
