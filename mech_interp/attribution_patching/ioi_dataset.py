...
def prepare_bisymmetric_prompts_and_answers(n_templates, n_pairs):
    """
    Generate prompts with bisymmetric structure using multiple templates.
    
    Args:
        n_templates: Number of different templates to use
        n_pairs: Number of name pairs for each template
        
    Returns:
        all_prompts: List of prompts with different name arrangements
        answers: List of tuples with (correct_answer, incorrect_answer)
    """
    # Create base datasets for different templates
    # Number of different templates to use
    n_templates = n_templates 
    # Number of name pairs for each template
    n_pairs = n_pairs      

    # Create BABA prompts (B and A went somewhere. B gave something to A)
    dataset = IOIDataset(
        prompt_type="BABA",
        N=n_templates * n_pairs,
        symmetric=True, 
        nb_templates=n_templates, 
    )
