...
def generic_activation_patch(
    ...
    patching_metric: Callable[[Float[torch.Tensor, "batch pos d_vocab"]], Float[torch.Tensor, "batch"]],
...
def generic_activation_patch(
    ...
    """
    Modified version of generic_activation_patch that supports metrics returning per-prompt values.
    ...
    Args:
        ...
        patching_metric: A function from the model's output logits to a tensor of shape [batch_size]
        patch_setter: A function which acts on (corrupted_activation, index, clean_cache) to edit the activation
        activation_name: The name of the activation being patched
        index_axis_names: The names of the axes to iterate over
        index_df: The dataframe of indices
        return_index_df: Whether to return the dataframe of indices too

    Returns:
        patched_output: The tensor of the patching metric for each patch. 
                       If flattened_output is True, shape is [n_indices, batch_size]
                       Otherwise, shape is [*index_dims, batch_size]
    ...
    # Add batch dimension to the output tensor
    if flattened_output:
        patched_metric_output = torch.zeros((len(index_df), corrupted_tokens.shape[0]), device=model.cfg.device)
    else:
        patched_metric_output = torch.zeros((*index_axis_max_range, corrupted_tokens.shape[0]), device=model.cfg.device)
    ...
    for c, index_row in enumerate(tqdm((list(index_df.iterrows())))):
        ...
        # Get metric for this patch - now returns a tensor of shape [batch_size]
        patched_metric_value = patching_metric(patched_logits)
        
        # Store the per-prompt results
        if flattened_output:
            patched_metric_output[c] = patched_metric_value
        else:
            patched_metric_output[tuple(index)] = patched_metric_value
